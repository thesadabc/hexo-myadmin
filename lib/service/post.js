...
module.exports = class PostService {
    ...
    constructor(hexo, type) {
            ...
            this.hexo = hexo;
            this.type = type;
            this.draftDir = path.join(this.hexo.source_dir, '_drafts');
            this.postDir = path.join(this.hexo.source_dir, '_posts');
        }
        /**
         *  @return Promise
         */
     ...
     */
    create({ meta, content }) {
        var compiled = hfm.parse(['---', meta, '---'].join('\n'));
        ...
        return this.hexo.post.create(compiled)
            .then((file) => this.updateDB().then(() =>
                file.path.slice(this.hexo.source_dir.length).replace(/\\/g, '\/')
            ));
    }
    ...
    update(id, { meta, content }) {
        ...
        var compiled = hfm.parse(['---', meta, '---', content].join('\n'));
        compiled.updated = compiled.updated || new Date();
        ...
        return fs.writeFile(post.full_source, hfm.stringify(compiled))
            .then(() => this.updateDB().then(() =>
                post.full_source.slice(this.hexo.source_dir.length).replace(/\\/g, '\/')
            ));
    ...
    publish(id) {
        ...
        let post = this.detail(id);
        return fs.rename(post.full_source, path.join(this.postDir, path.basename(post.full_source))).then(() => this.updateDB());
    }
    ...
    unpublish(id) {
        ...
        return fs.exists(this.draftDir).then((exists) => {
            if (!exists) return fs.mkdir(this.draftDir);
        }).then(() => {
            return fs.rename(post.full_source, path.join(this.draftDir, path.basename(post.full_source))).then(() => this.updateDB());
        })
    }
}
